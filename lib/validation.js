...
const registerValidation = [
    body('email')
        ...
        .custom((value, { req }) => {
            const assignedRole = req.body.role === 'sponsor' ? 'sponsor' : 'alumni';
            if (assignedRole === 'alumni' && !/@.*\.westminster\.ac\.uk$/.test(value)) {
                throw new Error('Registration is restricted to Westminster alumni (@*.westminster.ac.uk).');
            }
            return true;
        })
        .normalizeEmail(),
...
const credentialValidation = [
    ...
    body('url').optional({ checkFalsy: true }).isURL().withMessage('Invalid URL format.').trim(),
    body('completion_date').optional({ checkFalsy: true }).isDate().withMessage('Invalid date format.'),
    validate
];

const degreeValidation = credentialValidation;
