...
// Selects the daily winner at 6 PM and sends out emails to the winner and losers
const selectWinningAlumnus = async () => {
    try {
        ...
        // Find the Highest Valid Bid for today
...
// Resets the win counts for everyone at the start of the month (mandatory cleanup)
...
// Deletes old sessions and tokens to keep the database tidy
...
// Winner Selection at 4:55 PM Daily
cron.schedule('55 16 * * *', () => {
    selectWinningAlumnus();
});

// Database Cleanup at Midnight Daily
cron.schedule('0 0 * * *', () => {
    cleanupDatabase();
});

// Monthly win count reset at Midnight on the 1st of every month
cron.schedule('0 0 1 * *', () => {
